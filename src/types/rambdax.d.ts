declare module 'rambdax' {
  declare namespace R {
    ...
    interface X {
      ...
      defaultToStrict<T>(fallback: T, ...inputs: Array<T>): T

      defaultToWhen<T>(fallback: T, fn: Pred<T>, ...inputs: Array<T>): T
      ...
      inject(toInjectAfterMarker: string, marker: string, input: string): string

      includesAny(targets: any[], stringOrList: string | any[]): boolean
      includesAny(targets: any[]): (stringOrList: string | any[]) => boolean

      includesType(targetType: RambdaTypes): (list: any[]) => boolean
      includesType(targetType: RambdaTypes, list: any[]): boolean
      ...
      isPromise(maybePromiseOrAsync: any): boolean

      isFunction(maybePromiseFunctionOrAsync: any): boolean
      ...
      partition<T>(rule: Predicate<T>, input: Array<T>): [Array<T>, Array<T>]
      partition<T>(
        rule: Predicate<T>
      ...
      produce<T>(conditions: any, input: any): T
      produce<T>(conditions: any): (input: any) => T

      promiseAllObject<T>(input: ObjectWithPromises): Promise<T>
      ...
      tryCatch<T>(fn: any, fallback: any): Async<T> | T
      ...
      when<T>(rule: Func<boolean> | boolean, ruleTrue: any): IdentityFunction<T>
      when<T>(
      ...
      whereEq(rule: Record<string, any>, input: any): boolean
      whereEq(rule: Record<string, any>): (input: any) => boolean
      ...
      allPass<T>(predicates: Predicate<T>[], input: any): boolean
      allPass<T>(predicates: Predicate<T>[]): (input: any) => boolean
      ...
      anyPass<T>(predicates: Predicate<T>[], input: any): boolean
      anyPass<T>(predicates: Predicate<T>[]): (input: any) => boolean
      ...
      defaultTo<T>(
        defaultValue: T,
        ...inputArguments: Array<null | undefined | T>
      ): T
